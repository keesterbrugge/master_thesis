...
% $ \tau^{\rm max} = \max_{v \in \mathcal V}\tau^{\rm end}_v$
% and the earliest start time as
% $ \tau^{\rm min} = \min_{v \in \mathcal V}\tau^{\rm start}_v$
% . \\
...
%
% \begin{align}
%   \sum_{v} x_{i,v}^k (\tau_{i,k}^{\rm visit} + \tau_{i,v,k}^{\rm travel}) \leq \sum_{v \in \mathcal V} (t_{i,v}^k - t_{v,i}^k),
...
% \begin{align}
%   \sum_{v \in \mathcal V} t^k_{v,i} - t^k_{i,j} +
...
%   \left(  \sum_{v \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} x^k_{r^{-,j}_m,v} -1 \right)
%   \tau^R
%   \leq
%   visitationStart(r^{+,j}_m, X,Y, T) , \\
...
%   \left(  \sum_{w \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} x^k_{u,w} -1 \right)
%   \tau^R
  \leq
...
% $$ 
% visitationStart(u,X,Y,T) - visitationStart(v,X,Y,T) \leq \tau^{\rm R} \sum_{w \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} x^k_{u,w}, 
% \quad (u,v) \in \mathcal{E}^{\rm pairs}
% $$

% $$ 
% visitationStart(v,X,Y,T) - visitationStart(u,X,Y,T) \geq + \tau^{\rm R} \sum_{w \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} x^k_{u,w}   , 
% \quad (u,v) \in \mathcal{E}^{\rm pairs}
% $$
...
% \begin{align}
%   \sum_{v \in \mathcal V} x^k_{v,i} \tau_i^{\rm start} - t^k_{i,j} +
%   (x^{k}_{i,j} - 1  ) \tau^{\rm max}   \leq
%   -x^{k}_{i,j} \left(
%   \tau^{\rm visit}_{i,k} + \tau^{\rm travel}_{i,j,k}
%   \right),    \quad i,j \in  V ,
...
% \begin{align}
%   \label{eq:time-hard-last}
%   \sum_{v \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} y^{k,l}_{i,j} (\tau_{i,k}^{\rm visit} + \tau_{i,v,l}^{\rm travel}) \leq \sum_{v \in \mathcal V} \sum_{k \in \mathcal F^{\rm lorry}} (t_{i,v}^k - x^k_{v,i} \tau_i^{\rm start}),
...
%
% \begin{align}
%   \sum_{v \in \mathcal V} x^k_{v,i} \tau_i^{\rm start} - t^k_{i,j} + (y^{k,l}_{i,j} - 1  ) \tau^{\rm max} \leq \begin{cases}
%
%  \end{cases} \\
%  \quad \ i,j \in  V ,
%  \ k \in \mathcal F^{\rm lorry} ,
...
% \sum_{j \in \mathcal V} \sum_{l \in \mathcal F^{\rm lorry} }
% \left(
...
% WITH
% \sum_{k \in \mathcal F^{\rm lorry}}
% \sum_{i \in V \setminus N^-  OR instead I could use \in \mathcal S \cup R^-}
% \sum_{l \in \mathcal F^{\rm trailer}}
